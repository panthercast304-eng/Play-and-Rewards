...
/* ---------------- ONE "Watch Ad" button (AdsGram, TADS, Monetag, Adsbitvex, GigaPub) ----------------
   Tapping it picks a random ad network. If that network has no ad (or
   errors out quickly) the next network is tried immediately, until one
   plays or all have failed.
   - AdsGram: coins arrive via its server postback (adsgram-reward Edge Function).
   - TADS / Monetag / Adsbitvex: no postback, so the app calls the
     record_ad_watch RPC after the ad finishes (server adds coins + counts the ad). */
const AD_COINS=100;
const FAST_FAIL_MS=8000; // failing faster than this = "no ad available" -> try next network
const TADS_WIDGET_ID="12224";
// Set to true once AdsGram approves the platform AND the block is Active in the dashboard.
// While false, AdsGram is skipped so users never see its "block is not active" error popup.
const ADSGRAM_ENABLED=false;
let adsgramController=null, tadsController=null, tadsPending=null;

try{
  if(window.Adsgram){ adsgramController=window.Adsgram.init({blockId:"49052"}); }
  else console.warn("window.Adsgram is undefined — SDK script did not load.");
...
async function creditAdCoins(source){
  // Server function record_ad_watch(): adds the coins, counts the ad toward
  // ads_watched + the referrer's 10-ad progress, logs the ledger, and enforces
  // a rate limit / daily cap.
  if(!cur) return;
...
const AD_NETWORKS=[
  { name:"AdsGram",
    ready:()=>ADSGRAM_ENABLED&&!!adsgramController,
    // AdsGram rejects with {state}. "playing"/"destroy" = the user closed a real ad.
    userClosed:e=>!!(e&&(e.state==="playing"||e.state==="destroy")),
    show:async()=>{
      await adsgramController.show();
      setTimeout(async()=>{ await refreshCurrent(); render(); },3000);
      return "Ad watched — coins will appear in a few seconds.";
    } },
  { name:"TADS",
    ...
    show:()=>new Promise((resolve,reject)=>{
      let got=false;
      tadsPending={
        reward:()=>{ got=true; creditAdCoins("tads_ad").then(()=>resolve(AD_DONE_MSG)).catch(reject); },
        fail:()=>reject(new Error("tads_no_ad"))
      };
      Promise.resolve(tadsController.showAd())
        .then(()=>{ setTimeout(()=>{ if(!got&&tadsPending){ tadsPending=null; reject(new Error("tads_no_reward")); } },1500); })
        .catch(e=>{ tadsPending=null; reject(e); });
    }) },
  { name:"Monetag",
    ready:()=>typeof show_11834570==="function",
    show:()=>show_11834570().then(()=>creditAdCoins("monetag_ad")).then(()=>AD_DONE_MSG) },
  { name:"Adsbitvex",
    ready:()=>typeof window.showadsbitvex==="function",
    show:()=>window.showadsbitvex().then(()=>creditAdCoins("adsbitvex_ad")).then(()=>AD_DONE_MSG) },
  { name:"GigaPub",
    ready:()=>typeof window.showGiga==="function",
    show:()=>window.showGiga().then(()=>creditAdCoins("gigapub_ad")).then(()=>AD_DONE_MSG) }
];
...
async function tryNetwork(net){
  const t0=Date.now();
  try{ return {ok:true,msg:await net.show()}; }
  catch(e){
    console.log(net.name+" ad failed:",e);
    if(e&&e.stopMsg) return {ok:false,stopMsg:e.stopMsg};
    const closed=net.userClosed?net.userClosed(e):(Date.now()-t0)>=FAST_FAIL_MS;
    if(closed) return {ok:false,stopMsg:"Ad closed early — no coins earned. Tap to try again."};
    return {ok:false}; // no ad -> caller moves to the next network
  }
}
...
async function watchAd(){
  ...
  try{
    const nets=shuffle(AD_NETWORKS.filter(n=>n.ready()));
    if(!nets.length){ if(msg)msg.textContent="Ad system not loaded yet — please try again in a moment."; return; }
    for(const net of nets){
      if(msg)msg.textContent="Loading ad…";
      const r=await tryNetwork(net);
      if(r.ok){ if(msg)msg.textContent=r.msg; return; }
      if(r.stopMsg){ if(msg)msg.textContent=r.stopMsg; return; }
    }
