...
/* ---------------- OnClickA TMA rewarded video ----------------
   initCdTma() returns a SHOW function; calling it returns a Promise that
   resolves once the ad has played. No server postback is wired up, so we
   credit coins client-side through the same secured sync_coins RPC. */
const ONCLICKA_AD_COINS=100;
const ONCLICKA_SPOT_ID="6152266";
...
try{
  if(typeof window.initCdTma==="function"){
    ...
    console.warn("window.initCdTma is undefined — OnClickA SDK script did not load.");
  }
}catch(e){ console.warn("OnClickA SDK not available:",e); }

if($("onclickaBtn"))$("onclickaBtn").onclick=()=>{
  const btn=$("onclickaBtn"), msg=$("onclickaMsg");
  if(typeof onclickaShow!=="function"){
    alert("Ad system not loaded yet — please try again in a moment.");
    return;
  }
  btn.disabled=true; btn.textContent="Loading ad…";
  ...
  onclickaShow().then(()=>{
    if(!cur) return;
    return window.PFY_GameBridge.syncBalance(cur.coins+ONCLICKA_AD_COINS,"onclicka_ad").then(()=>{
      if(msg)msg.textContent="Ad watched — +"+ONCLICKA_AD_COINS+" coins added!";
    });
  }).catch((e)=>{
    console.log("OnClickA ad error:",e);
    if(msg)msg.textContent="No ad available right now — try again shortly.";
  }).finally(()=>{
    btn.disabled=false; btn.textContent="🎬 Watch OnClickA Ad — +"+ONCLICKA_AD_COINS+" Coins";
  });
};

/* ---------------- Adsbitvex rewarded ad ----------------
   window.showadsbitvex() shows a full-screen reward ad (15s countdown)
   and returns a Promise that resolves once the user has watched it.
   No server postback is wired up, so we credit coins client-side through
   the same secured sync_coins RPC used by the other ad buttons. */
const ADSBITVEX_AD_COINS=100;

if($("adsbitvexBtn"))$("adsbitvexBtn").onclick=()=>{
  const btn=$("adsbitvexBtn"), msg=$("adsbitvexMsg");
  ...
  window.showadsbitvex().then(()=>{
    if(!cur) return;
    return window.PFY_GameBridge.syncBalance(cur.coins+ADSBITVEX_AD_COINS,"adsbitvex_ad").then(()=>{
      if(msg)msg.textContent="Ad watched — +"+ADSBITVEX_AD_COINS+" coins added!";
    });
  }).catch((e)=>{
    console.log("Adsbitvex ad error:",e);
    if(msg)msg.textContent="No ad available right now — try again shortly.";
  }).finally(()=>{
    btn.disabled=false; btn.textContent="🎬 Watch Adsbitvex Ad — +"+ADSBITVEX_AD_COINS+" Coins";
  });
};
