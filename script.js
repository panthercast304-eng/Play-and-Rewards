...
const sb = window.supabase.createClient(
  "https://gpqhsclilrzlxrxlrjxv.supabase.co",
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdwcWhzY2xpbHJ6bHhyeGxyanh2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODg3OTM5OTksImV4cCI6MjEwNDM2OTk5OX0.Nj9VKVwpLCvWh_EkHtNK0X7W1UHVGRH2tIo1mEm47II"
);
...
(async function boot(){
 ...
 if(sessionErr){ alert("DEBUG: getSession error -> "+sessionErr.message); }
 if(!session){ alert("DEBUG: No session found in storage on this load."); }
 if(session){
   alert("DEBUG: Session found for "+session.user.email);
   const {data:profile,error:profileErr}=await sb.from("profiles").select("*").eq("id",session.user.id).single();
   if(profileErr){ alert("DEBUG: Profile fetch error -> "+profileErr.message); }
   if(profile){
     cur={...profile, referredUsers:[], ledger:[], withdrawals:[]};
     await refreshCurrent();
     $("auth").hidden=true;$("app").hidden=false;$("nav").hidden=false;
