...
/* ============================================================
   ...
   Auth, coins, referrals, ledger and withdrawals now live in
   Supabase (see config.js for URL/anon key). Ad-watch and
   referral-progress updates should ideally be verified through
   a Supabase Edge Function / Postgres function rather than the
   plain client update()s used here, since a user could otherwise
   edit values from devtools. This file keeps the client-side
   calls simple to get you running; harden the write paths with
   RPC functions before you rely on this for real payouts.
   ============================================================ */
...
async function ledgerAdd(type,amount,status){
  await sb.from("ledger").insert({user_id:cur.id,type,amount,status:status||"verified"});
}
...
async function applyReferralCode(code,silent){
 ...
 const {data:owner,error:ownerErr}=await sb.from("profiles").select("id").eq("ref_code",code).single();
 if(ownerErr||!owner){setMsg("That referral code isn't valid.");return}

 const {error:refErr}=await sb.from("referrals").insert({referrer_id:owner.id, referred_id:cur.id, status:"pending", ads_progress:0});
 if(refErr){setMsg("Could not apply referral code — try again.");return}

 await sb.from("profiles").update({applied_ref:code}).eq("id",cur.id);
 cur.applied_ref=code;
 setMsg("Referral code applied — pending verification.");
 await refreshCurrent(); render();
}

async function progressReferredAds(){
 // when THIS user watches an ad, tick progress on whichever account referred them
 if(!cur.applied_ref)return;
 const {data:refRow}=await sb.from("referrals").select("*").eq("referred_id",cur.id).eq("status","pending").maybeSingle();
 if(!refRow)return;
 const newProgress=(refRow.ads_progress||0)+1;
 const update={ads_progress:newProgress};
 if(newProgress>=10) update.status="eligible";
 await sb.from("referrals").update(update).eq("id",refRow.id);
}

async function claimReferralBonus(){
 const {data:eligible}=await sb.from("referrals").select("*").eq("referrer_id",cur.id).eq("status","eligible").limit(1).maybeSingle();
 if(!eligible){return}
 await sb.from("referrals").update({status:"claimed"}).eq("id",eligible.id);
 const newCoins=cur.coins+1200;
 await sb.from("profiles").update({coins:newCoins}).eq("id",cur.id);
 await ledgerAdd("Referral Bonus",1200);
 await refreshCurrent(); render();
}
...
async function startAd(i,btn){
 ...
 try{
   ...
   const newCoins=cur.coins+200, newAdsWatched=cur.ads_watched+1;
   await sb.from("profiles").update({coins:newCoins,ads_watched:newAdsWatched}).eq("id",cur.id);
   await ledgerAdd("Ad Reward",200);
   await progressReferredAds();
...
async function startRealVideoAd(btn){
 ...
 try{
   ...
   const newCoins=cur.coins+REAL_AD_COINS, newAdsWatched=cur.ads_watched+1;
   await sb.from("profiles").update({coins:newCoins,ads_watched:newAdsWatched}).eq("id",cur.id);
   await ledgerAdd("Ad Reward (Real Video)",REAL_AD_COINS);
   await progressReferredAds();
...
async function gp(n,label){
 const newPoints=cur.game_points+n;
 await sb.from("profiles").update({game_points:newPoints}).eq("id",cur.id);
 await refreshCurrent(); render();
...
function daily(){
 ...
 $("dailySubmit").onclick=async ()=>{
   ...
   if(answer.includes(val)){
     await sb.from("profiles").update({daily_last_claim:todayKey()}).eq("id",cur.id);
     await refreshCurrent();
     $("dailyMsg").textContent="Correct!";
     await gp(100,"Daily Challenge");
     daily();
   }else{
...
async function submitWithdraw(){
 ...
 await sb.from("profiles").update({coins:newCoins}).eq("id",cur.id);
 await sb.from("withdrawals").insert({user_id:cur.id,amount,method,upi,status:"pending review"});
 await ledgerAdd("Withdrawal Request",-amount,"pending");
 await refreshCurrent();
