...
/* ---------------- Monetag (Telegram) rewarded interstitial ad ----------------
   Second ad option, separate from the HilltopAds VAST player above.
   Uses Monetag's SDK function show_11834570() loaded in <head>.
   Same coin reward (award_ad_watch RPC) as the main real-video-ad button. */
async function startTelegramAd(btn){
 const msg=$("telegramAdMsg");
 ...
 try{
   ...
   btn.disabled=false; btn.textContent="📱 Watch Real Video Ad — Telegram +"+REAL_AD_COINS+" Coins";
 }
}
if($("telegramAdBtn"))$("telegramAdBtn").onclick=()=>startTelegramAd($("telegramAdBtn"));
