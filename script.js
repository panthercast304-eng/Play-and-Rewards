...
/* ---------------- real rewarded video ad (HilltopAds via Supabase vast-proxy) ----
   ...
   video's "ended" event, so it's still technically fakeable from
...
// Fetch a VAST document through the Supabase proxy. `overrideUrl` is used
// when following a Wrapper's <VASTAdTagURI> to a second-generation tag.
...
// Pick the best playable <MediaFile> from an InLine VAST document.
...
// Resolve a VAST tag to a playable media file URL, following Wrapper
// redirects (each hop goes back through the proxy so CORS stays satisfied).
...
function playRealVastAd(){
 return new Promise(async (resolve,reject)=>{
   ...
   // Mobile browsers sometimes interrupt an in-flight play() (e.g. a stray
   // tap, or the element reloading) with "AbortError: interrupted by pause()".
   // That's not a real failure — retry a couple times before giving up.
