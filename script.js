...
/* ---------------- Monetag rewarded interstitial ----------------
   Like TADS, Monetag's show_ZONEID() promise resolves once the ad has
   actually been watched — no server postback wired up (yet), so we
   credit coins client-side through the same secured sync_coins RPC. */
const MONETAG_AD_COINS=100;

if($("monetagBtn"))$("monetagBtn").onclick=()=>{
  const btn=$("monetagBtn"), msg=$("monetagMsg");
  ...
  show_11834570().then(()=>{
    if(!cur) return;
    return window.PFY_GameBridge.syncBalance(cur.coins+MONETAG_AD_COINS,"monetag_ad").then(()=>{
      if(msg)msg.textContent="Ad watched — +"+MONETAG_AD_COINS+" coins added!";
    });
  }).catch((e)=>{
    console.log("Monetag ad error:",e);
    if(msg)msg.textContent="No ad available right now — try again shortly.";
  }).finally(()=>{
    btn.disabled=false; btn.textContent="🎬 Watch Monetag Ad — +"+MONETAG_AD_COINS+" Coins";
  });
};

/* ---------------- HilltopAds rewarded VAST video ----------------
   Fetches the VAST tag through the vast-proxy Edge Function (bypasses
   CORS), follows one level of VAST Wrapper redirect if present, plays
   the returned MediaFile in a fullscreen <video>, fires the ad
   network's own Impression/Tracking pixels, and only credits coins if
   the video plays to the end (closing early = no reward). */
const HILLTOP_AD_COINS=150;
const HILLTOP_VAST_URL="https://surefootedpause.com/damKFyz.ddGDNvvnZPGqUx/Yefm/9/uLZVU/lfk/P/TWc/0TNFDTAUzmOLDqkEtkNEzyQe0xMkD/Mg5TMQwx";
const VAST_PROXY="https://gpqhsclilrzlxrxlrjxv.supabase.co/functions/v1/vast-proxy";

function firePixels(urls){
  (urls||[]).forEach(u=>{ try{ const img=new Image(); img.src=u; }catch(e){} });
}

function parseVastXml(xmlText){
  const doc=new DOMParser().parseFromString(xmlText,"text/xml");
  if(doc.querySelector("parsererror")) throw new Error("Invalid VAST XML");
  const wrapperUri=doc.querySelector("VASTAdTagURI");
  if(wrapperUri && wrapperUri.textContent.trim()){
    return {wrapper:wrapperUri.textContent.trim()};
  }
  const mediaFile=doc.querySelector("MediaFile");
  if(!mediaFile || !mediaFile.textContent.trim()) return null;
  const impressions=Array.from(doc.querySelectorAll("Impression")).map(n=>n.textContent.trim()).filter(Boolean);
  const completeTracking=Array.from(doc.querySelectorAll('Tracking[event="complete"]')).map(n=>n.textContent.trim()).filter(Boolean);
  return {mediaUrl:mediaFile.textContent.trim(),impressions,completeTracking};
}

async function resolveVast(url,depth){
  if(depth>3) throw new Error("Too many VAST wrapper redirects");
  const res=await fetch(VAST_PROXY+"?url="+encodeURIComponent(url));
  if(!res.ok) throw new Error("vast-proxy HTTP "+res.status);
  const xml=await res.text();
  const parsed=parseVastXml(xml);
  if(!parsed) throw new Error("Empty VAST response — no ad available");
  if(parsed.wrapper) return resolveVast(parsed.wrapper,depth+1);
  return parsed;
}

async function startHilltopAd(btn){
  const overlay=$("vastOverlay"), video=$("vastVideo"), status=$("hilltopMsg"), vastStatus=$("vastStatus");
  btn.disabled=true; btn.textContent="Loading ad…";
  if(status)status.textContent="Fetching video ad…";
  try{
    const ad=await resolveVast(HILLTOP_VAST_URL,0);
    firePixels(ad.impressions);
    video.src=ad.mediaUrl;
    overlay.hidden=false;
    if(vastStatus)vastStatus.textContent="Watch the full video to earn coins.";
    video.play().catch(()=>{});
    const outcome=await new Promise((resolve,reject)=>{
      video.onended=()=>resolve("ended");
      video.onerror=()=>reject(new Error("Video playback failed"));
      $("vastCloseBtn").onclick=()=>resolve("closed");
    });
    overlay.hidden=true;
    video.pause(); video.src="";
    if(outcome==="ended"){
      firePixels(ad.completeTracking);
      if(cur){
        await window.PFY_GameBridge.syncBalance(cur.coins+HILLTOP_AD_COINS,"hilltop_ad");
      }
      if(status)status.textContent="Ad watched — +"+HILLTOP_AD_COINS+" coins added!";
    }else{
      if(status)status.textContent="Ad closed early — no coins awarded.";
    }
  }catch(e){
    console.log("HilltopAds VAST error:",e);
    overlay.hidden=true;
    if(status)status.textContent="No ad available right now — try again shortly.";
  }finally{
    btn.disabled=false; btn.textContent="🎬 Watch HilltopAds Video — +"+HILLTOP_AD_COINS+" Coins";
  }
}
if($("hilltopBtn"))$("hilltopBtn").onclick=()=>startHilltopAd($("hilltopBtn"));
