...
/* ---------------- real rewarded video ad (HilltopAds via Google IMA SDK) ----
   ...
   Google's IMA SDK is the standard, CORS-safe way to request and play a
   VAST ad client-side — it's designed by Google specifically to talk to
   third-party ad servers from the browser. We load it once, then use it
   to play the same HilltopAds VAST tag as before.
   NOTE: still no server-side postback — completion is detected via the
   ALL_ADS_COMPLETED event, so it's still technically fakeable from
   devtools. CPAlead/Torox's Video Wall remains the path to a real
   server-verified rewarded ad. */
const HILLTOP_VAST_URL="https://surefootedpause.com/dLmkFgz.dAGFNrvUZPGKUk/iepmn9luZZ/UBlhkcP_TtcXO/MgjHIEOIM/TNcitFNQz_QbyrMxjPQIyuMtQN";
const REAL_AD_COINS=200;

function loadImaSdk(){
 return new Promise((resolve,reject)=>{
   if(window.google && window.google.ima){resolve();return}
...
function playRealVastAd(){
 return new Promise(async (resolve,reject)=>{
   try{ await loadImaSdk(); }catch(e){ reject(e); return }

   const outer=$("realAdContainer"), adDiv=$("imaAdContainer"), video=$("realAdVideo");
   if(!outer||!adDiv||!video){reject(new Error("no player on page"));return}

   outer.hidden=false; adDiv.style.display="block";

   function cleanup(){ outer.hidden=true; adDiv.innerHTML=""; adDiv.style.display="none"; }

   try{
     google.ima.settings.setDisableCustomPlaybackForIOS10Plus(true);
     const adDisplayContainer=new google.ima.AdDisplayContainer(adDiv, video);
     const adsLoader=new google.ima.AdsLoader(adDisplayContainer);

     adsLoader.addEventListener(google.ima.AdErrorEvent.Type.AD_ERROR, (e)=>{
       cleanup(); reject(new Error(e.getError().toString()));
     }, false);

     adsLoader.addEventListener(google.ima.AdsManagerLoadedEvent.Type.ADS_MANAGER_LOADED, (loadedEvt)=>{
       const manager=loadedEvt.getAdsManager(video);
       manager.addEventListener(google.ima.AdErrorEvent.Type.AD_ERROR, (e)=>{
         cleanup(); reject(new Error(e.getError().toString()));
       });
       manager.addEventListener(google.ima.AdEvent.Type.ALL_ADS_COMPLETED, ()=>{ cleanup(); resolve(); });
       manager.addEventListener(google.ima.AdEvent.Type.CONTENT_RESUME_REQUESTED, ()=>{ cleanup(); resolve(); });
       try{
         adDisplayContainer.initialize();
         const w=adDiv.clientWidth||400, h=adDiv.clientHeight||225;
         manager.init(w,h,google.ima.ViewMode.NORMAL);
         manager.start();
       }catch(adError){ cleanup(); reject(adError); }
     }, false);
...
async function startRealVideoAd(btn){
 ...
 try{
   ...
   cur.coins+=REAL_AD_COINS; cur.adsWatched++;
   ledgerAdd(cur,"Ad Reward (Real Video)",REAL_AD_COINS);
   users[cur.email]=cur; saveUsers(users);
   progressReferredAds();
