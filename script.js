...
/* ---------------- ONE "Watch Ad" button ----------------
   ...
   - TADS / Monetag / Adsbitvex: no postback, so coins are credited
     client-side through the secured sync_coins RPC (server-capped + logged). */
const AD_COINS=100;
...
try{
  if(window.Adsgram){ adsgramController=window.Adsgram.init({blockId:"48752"}); }
...
function creditAdCoins(source){
  if(!cur) return Promise.resolve();
  return window.PFY_GameBridge.syncBalance(cur.coins+AD_COINS,source).catch(()=>{
    const err=new Error("credit_failed");
    err.stopMsg="Ad watched, but the coins could not be saved — please check your balance.";
    throw err;
  });
}
