...
/* ---------------- real rewarded video ad (HilltopAds VAST) ----------------
   This plays an ACTUAL video ad (not a simulated timer) and credits coins
   when the video finishes. NOTE: there is still no server-side postback
   here — completion is detected client-side via the video's "ended"
   event, same as the rest of this demo. It's real ad inventory, but the
   reward trigger is still fakeable (e.g. from devtools). CPAlead/Torox's
   Video Wall (once approved) is the path to a real server-verified
   rewarded ad. */
...
async function startRealVideoAd(btn){
 const container=$("realAdContainer"), video=$("realAdVideo"), msg=$("realAdMsg");
 ...
 try{
   ...
   container.hidden=false; btn.hidden=true;
   if(msg)msg.textContent="Watch the full ad to earn your coins.";
   video.play();
   video.onended=()=>{
     container.hidden=true; btn.hidden=false;
     btn.disabled=false; btn.textContent="📺 Watch Real Video Ad — +"+REAL_AD_COINS+" Coins";
   ...
   video.onerror=()=>{
     container.hidden=true; btn.hidden=false;
     btn.disabled=false; btn.textContent="📺 Watch Real Video Ad — +"+REAL_AD_COINS+" Coins";
     if(msg)msg.textContent="Ad failed to play — try again.";
   };
 }catch(e){
   if(msg)msg.textContent="Ad failed to load — try again.";
   btn.disabled=false; btn.textContent="📺 Watch Real Video Ad — +"+REAL_AD_COINS+" Coins";
 }
}
